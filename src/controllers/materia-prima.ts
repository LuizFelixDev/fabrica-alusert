...
export const createMateriaPrima = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { nome, descricao, unidade_medida, quantidade_estoque, valor_unitario, estoque_minimo } = req.body;
    if (!nome || !unidade_medida) {
      return res.status(400).json({ error: 'Campos nome e unidade_medida são obrigatórios' });
    }

    const result = await pool.query(
      `INSERT INTO materias_primas (nome, descricao, unidade_medida, quantidade_estoque, valor_unitario, estoque_minimo)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [
        nome,
